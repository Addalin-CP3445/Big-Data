...
if __name__ == "__main__":
    ...
    time_of_day_df = fully_joined_df.select("time_of_day", "trip_length", "driver_total_pay")
    ...
    avg_trip_length_df = spark.createDataFrame(data_trip_length, ["time_of_day", "average_trip_length"])

    # creating list of tuples of average driver_total_pay per time_of_day
    data_driver_total_pay = [
        ("evening", evening_df.select(avg("driver_total_pay")).collect()[0][0]),
        ("morning", morning_df.select(avg("driver_total_pay")).collect()[0][0]),
        ("afternoon", afternoon_df.select(avg("driver_total_pay")).collect()[0][0]),
        ("night", night_df.select(avg("driver_total_pay")).collect()[0][0])
    ]

    # creating new dataframe with time_of_day and average_drive_total_pay
    avg_driver_total_pay_df = spark.createDataFrame(data_driver_total_pay, ["time_of_day", "average_driver_total_pay"])

    # Joining the DataFrames and creating a new column 'average_earning_per_mile'
    joined_df = avg_trip_length_df.join(avg_driver_total_pay_df, "time_of_day", "inner") \
        .withColumn("average_earning_per_mile", col("average_driver_total_pay") / col("average_trip_length"))

    # dropping unnecessary columns 
    joined_df = joined_df.drop("average_trip_length").drop("average_driver_total_pay")
