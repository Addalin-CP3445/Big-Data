...
if __name__ == "__main__":
    ...
    pickup_borough_df = fully_joined_df.select("Dropoff_Borough", "date")

    # Define column names for new dataframe
    column_names = ["Dropoff_Borough", "Month", "trip_count"] 
    ...
    for i in range(1, 6):
        ...
        trip_count_df = filtered_by_month_df.groupBy("Dropoff_Borough").agg(count("*").alias("trip_count")) # Counting trip count for each dropoff borough and storing values in a new column named 'trip_count'
        ...
        total_trip_count_df = total_trip_count_df.union(top_5_trip_count_df.select("Dropoff_Borough", "Month", "trip_count")) # Union of the main dataframe with the dataframe produced each iteration
