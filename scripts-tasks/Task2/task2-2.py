...
if __name__ == "__main__":
    ...
    rideshare_df = fully_joined_df.select("business", "date", "rideshare_profit")

    # Extracting month from the 'date' column
    rideshare_df = rideshare_df.withColumn("month", month(col("date")))

    # Casting the 'rideshare_profit' column to FloatType
    rideshare_df = rideshare_df.withColumn("rideshare_profit", col("rideshare_profit").cast(FloatType()))

    # Creating new 'business_month' column
    rideshare_df = rideshare_df.withColumn("business_month", concat(col("business"), col("month")))

    # Adding the platform's profits by each business-month
    rideshare_profit_df= rideshare_df.groupBy("business_month").sum("rideshare_profit").withColumnRenamed("sum(rideshare_profit)", "platform_profits")

    # repartitioning the dataframe to a single file
    rideshare_profit_df = rideshare_profit_df.repartition(1)

    # writing as csv file to the shared bucket 
    rideshare_profit_df.write.csv(shared_bucket_path + "task_2_part_2", header=True)
