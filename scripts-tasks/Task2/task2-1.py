...
if __name__ == "__main__":
    ...
    pickup_borough_df = fully_joined_df.select("business", "date")

    # Define column names for new dataframe
    column_names = ["business", "Month", "trip_count"] 
    ...
    for i in range(1, 6):
        ...
        trip_count_df = filtered_by_month_df.groupBy("business").agg(count("*").alias("trip_count")) # Counting trip count for each business and storing values in a new column named 'trip_count' 
        trip_count_df = trip_count_df.withColumn("Month", lit(i))  # Adding Month column with the month value of the current iteration 
        total_trip_count_df = total_trip_count_df.union(trip_count_df.select("business", "Month", "trip_count")) # Union of the main dataframe with the dataframe produced each iteration

    total_trip_count_df = total_trip_count_df.orderBy(col("Month")) # ordering by month
    total_trip_count_df = total_trip_count_df.repartition(1) # repartitioning the dataframe to a single file
    total_trip_count_df.write.csv(shared_bucket_path + "task_2_part_1", header=True) # writing as csv file to the shared bucket 
