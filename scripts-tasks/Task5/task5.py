...
from pyspark.sql.functions import from_unixtime, avg, col, dayofmonth, month
...
if __name__ == "__main__":
    ...
    filtered_df = fully_joined_df.select("date", "request_to_pickup")

    # filtering dataframe by month of January 
    jan_df = filtered_df.filter(month("date") == 1)

    # renaming the 'date' column as 'day'
    jan_df = jan_df.withColumn("day", dayofmonth(col("date")))

    # creating a new column with values of average request_to_pickup grouped by day 
    avg_waiting_time_df = jan_df.groupBy("day").agg(avg("request_to_pickup").alias("average_waiting_time"))

    # ordering the dataframe by day 
    avg_waiting_time_df = avg_waiting_time_df.orderBy("day")

    # repartitioning the dataframe to a single file 
    avg_waiting_time_df = avg_waiting_time_df.repartition(1)

    # writing as csv file to the shared bucket 
    avg_waiting_time_df.write.csv(shared_bucket_path + "avg_waiting_time", header=True)
